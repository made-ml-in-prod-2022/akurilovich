...
with DAG(
    ...
    model_sensor = FileSensor(
        ...
        filepath=os.path.join("/opt/airflow/" + Variable.get("DIR_MODEL"), MODEL_FILENAME),
        poke_interval=5,
        retries=200,
    )
    ...
    inference = DockerOperator(
        ...
        command=f"--dir-in-data {DIR_DATA} --dir-in-model {Variable.get('DIR_MODEL')} --dir-out-predictions {DIR_PREDICTIONS}",
        network_mode="bridge",
        do_xcom_push=False,
        mount_tmp_dir=False,
        mounts=[MOUNT],
